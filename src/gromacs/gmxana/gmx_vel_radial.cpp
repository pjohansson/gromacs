...
// in a grid of input spacing. vr is positive along the increasing
// radial axis.
static Grid<double> calc_radial_mass(const t_topology       *top,
                                     int                     ePBC,
                                     const char             *fn,
                                     const int               axis,
                                     const rvec              origin,
                                     ...
                                     const gmx_output_env_t *oenv)
{
    ...
    if (!read_first_frame(oenv, &status, fn, &frame, TRX_NEED_X))
    {
        gmx_fatal(FARGS, "Could not read trajectory");
    }
    ...
    /* Grid is ordered as: grid[iz][ir]
       Contains: mass (or number) values
       counts of additions are stored separately for averaging */
    Grid<double> mass(nh, std::vector<double> (nr, 0.0));
    ...
    for (size_t ih = 0; ih < nh; ++ih)
    {
        for (size_t ir = 0; ir < nr; ++ir)
        {
            ...
            if (count > 0)
            {
                mass[ih][ir] /= static_cast<double>(count);
            }
        }
    }
...
{
    const char       *desc[] = {
        "[THISMODULE] computes the radial density distribution of samples ",
        "in a simulation.",
        "[PAR]",
    ...
    const char *sample_opt[eSampleN + 1] = { nullptr, "velocity", "mass", nullptr };
    ...
    t_pargs           pa[] = {
        ...
        { "-sample",   FALSE, etENUM, {&sample_opt},
          "Parameter to sample" }
    };
...
#define NFILE asize(fnm)
    ...
    auto param = Parameter::Velocity;
    switch (nenum(sample_opt))
    {
        case eVelocity:
            break;
        case eMass:
            param = Parameter::Mass;
            break;
    }
    ...
    switch (param)
    {
        ...
        case Parameter::Mass:
        {
            const auto mass_components = calc_radial_mass(
                top, ePBC, ftp2fn(efTRX, NFILE, fnm), axis, origin, bUseOrigin, spacing,
                gnx, grpindex, oenv);
            print_grid_singles(
                mass_components,
