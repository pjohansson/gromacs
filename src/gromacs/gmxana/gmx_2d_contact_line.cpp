...
struct CLConf {
    CLConf(t_pargs    pa[],
        ...
        :algorithm{ static_cast<Algorithm>(alg_in) },
         ...
         dr_min{ opt2parg_real("-drmin", pasize, pa) },
         dr_min2{ dr_min * dr_min },
         dz { opt2parg_real("-dz", pasize, pa) },
                 ...
                 const CLConf      &conf)
{
    ...
    constexpr real dx_ball = 0.02;
    constexpr real dy_ball = 0.02;
    ...
    while (y <= conf.rmax[YY])
    {
        ...
        while (x >= conf.rmin[XX])
        {
            ...
            {
                // This currently checks against OW (input group)
                // AND the two following HW atoms for every index. This
                // should be done through a group!

                for (unsigned sol_index = 0; sol_index < 3; ++sol_index)
                {
                    const auto dx = x - x0[index + sol_index][XX];
                    const auto dy = y - x0[index + sol_index][YY];
            ...
            // If an atom was detected within the ball, find which candidate
            // was the closest one to it. This is the contact-line molecule
            // for this position.
                          ...
                          const CLConf    &conf)
{
    ...
    {
        gmx_warning("Current and previous interface are of different size "
                    "(prev: %d, current: %d). This should not happen.",
                    previous.rolled_xs.size(), current.rolled_xs.size());
    }
        ...
        && (xprev != previous.rolled_xs.cend()))
    {
        ...
        if (dx >= conf.dx)
        {
            ...
            const auto dr2 = std::pow(x2[XX] - x1[XX], 2) + std::pow(x2[YY] - x1[YY], 2);
            const auto dz = x2[ZZ] - x1[ZZ];

            if (dr2 >= conf.dr_min2 || (conf.dz > 0.0 && dz >= conf.dz))
            {
            ...
            {
                fprintf(stderr, "Discarded a sidestepping-hop (dr: %.2f, dz: %.2f)\n", sqrt(dr2), dz);
            }
#endif
        }
...
static IndexSet
at_previous_contact_line(const IndexSet  &cl_indices,
                         ...
                         const CLConf    &conf)
{
    ...
    switch (conf.algorithm)
    {
        case Algorithm::ContactLine:
            prev_contact_line = find_shared_indices(cl_indices, previous.contact_line);
            break;

        case Algorithm::Bottom:
            prev_contact_line = find_shared_indices(cl_indices, previous.bottom);
            break;

        case Algorithm::Hops:
            {
                const auto shared = find_shared_indices(cl_indices, previous.bottom);
                    ...
                    const IndexVec &num_advanced)
{
    ...
    fprintf(stdout,
            ...
            fraction, err, static_cast<unsigned>(sum_previous), static_cast<unsigned>(sum_total));
...
gmx_contact_line(int argc, char *argv[])
{
    ...
    t_pargs pa[] = {
        ...
        { "-drmin", FALSE, etREAL, { &dr_min },
          "Minimum distance of atom movement for contact line advancement (conditional *or* along with dz)." },
        { "-dz", FALSE, etREAL, { &dz },
          "Distance of atom movement along z for contact line advancement (conditional *or* along with drmin)." },
        ...
        { "-al" , FALSE, etENUM, { &algorithm },
          "Algorithm for determining elegibility of atoms (`hops` is best)." },
...
#define NFILE asize(fnm)
    ...
    gmx_warning("This program is (currently) hard-coded to detect H2O"
                "interfaces, using a group of the OW atoms only.");
